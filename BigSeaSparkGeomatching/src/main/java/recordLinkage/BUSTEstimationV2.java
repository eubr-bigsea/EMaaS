 ...
 * In this version (V2), this class includes the bus tickets file to the output.
 * 
 * @author Andreza
 *
 */
...
//		SparkConf sparkConf = new SparkConf().setAppName("BUSTEstimationV2").setMaster("local");
		SparkConf sparkConf = new SparkConf().setAppName("BUSTEstimationV2");
	...
	private static JavaRDD<String> execute(JavaSparkContext context, JavaRDD<String> bulmaOutputString,
		...
		JavaRDD<String> rddOutput = rddMapInterpolation
				.flatMap(new FlatMapFunction<Tuple2<String, Iterable<Tuple2<String, String>>>, String>() {
							...
							if (!currentBusStop.equals("-")) {
								...
								if (!currentTimeString.equals("-")) {
									if (nextTimeString == null) {
										...
										int number = getNumberTicketsOfBusStop(currentBusCode, currentTimeString,
												nextTimeString);
										nextTimeString = currentTimeString;
					...
					private int getNumberTicketsOfBusStop(String currentBusCode, String currentTimeString,
