...
//	private static final int THRESHOLD_OUTLIERS_POINTS = 2;
	...
	public static void main(String[] args) {
		...
		if (args.length < 4) {
			System.err.println(
					"Usage: <shape file path> <GPS file path> <output path> "
							+ "<partitions number>");
			System.exit(1);
		}
...
//		JavaStreamingContext context = new JavaStreamingContext(sparkConf, Durations.seconds(batchDuration));
		 ...
		 * JavaDStream<String>
		 */
		JavaRDD<String> gpsStream = context.textFile(pathFileGPS, minPartitions)
				.mapPartitionsWithIndex(removeHeader, false).repartition(minPartitions);
...
//								}
					...
					private Tuple2<ShapePoint, Float> getClosestShapePoint(GPSPoint gpsPoint, ShapeLine shapeLine) {
						...
						for (GeoPoint shapePoint : shapeLine.getListGeoPoints()) {
							if (gpsPoint == null || shapePoint == null ) {
								System.out.println();
							}
							...
							if (listRetrocessingPoints != null) {
								for (int i = 0; i < listRetrocessingPoints.size(); i++) {
									Tuple2<ShapePoint, Float> closestPointRetrocessing = getClosestShapePoint(
											listRetrocessingPoints.get(i), trip.getShapeMatched());
									if (closestPointRetrocessing._2 > THRESHOLD_DISTANCE_BETWEEN_POINTS) {
										listRetrocessingPoints.get(i).setProblem(Problem.OUTLIER_POINT.getCode());
										trip.addOutlierInSequence(currentGPSPoint);
									} else {
										trip.cleanOutliersInSequenceList();
									}
									trip.addPointToPath(listRetrocessingPoints.get(i), closestPointRetrocessing._1,
											closestPointRetrocessing._2);
									listOutput.add(new Tuple3<GPSPoint, ShapePoint, Float>(listRetrocessingPoints.get(i),
...
//							}
		 ...
		 * Formats the output.
		 */
		JavaRDD<String> rddOutput = similarityOutput
		...
		rddOutput.saveAsTextFile(pathOutput);
